...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    (async ()=>{
      const res = await fetch(ENTRYPOINT+"/shows");
      if (res.status == 200) {
            ...
            <div className="flex flex-row" style={{height: '95%'}}>
              <div className="flex flex-wrap overflow-auto justify-center">
                {Array.isArray(shows) && shows.map((v,i)=>{return (
                  <ShowPreviewCard key={i} src={v.movie?.media[0]} name={v.movie?.name} tags={v.movie?.tags} date={v?.begin_date} onClick={()=>{router.push(v["@id"]?.slice(4))}}/>
                )})}
              </div>
            </div>
