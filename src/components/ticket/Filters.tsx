...
const FiltersBlock = () => {
    const {push: route, query: { id: queryId, client: queryClient, show: queryShow }} = useRouter();
    ...
    return (
        <div className="flex gap-1">
            ...
            <ShowFilter show={show} defaultValue={queryClient}/>
            <ClientFilter client={client} defaultValue={queryShow}/>
            <button onClick={()=>{
                route(`?${filterQueryBuilder({id: id.current, client: client.current, show: show.current})}`);
            }}>Search</button>
        </div>
    );
} 
